...
export const getSelectedCharacter = createAsyncThunk(
	'rickAndMorty/getSelectedCharacter',
	async (id, thunkAPI) => {
		try {
			return await characterService.getSelectedCharacter(id);
		} catch (error) {
			const message =
				(error.response &&
					error.response.data &&
...
const characterSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(getSelectedCharacter.pending, (state) => {
				state.isLoading = true;
			})
			.addCase(getSelectedCharacter.fulfilled, (state, action) => {
				state.isLoading = false;
				state.isSuccess = true;
				state.isError = false;
				state.selectedCharacter = action.payload;
			})
			.addCase(getSelectedCharacter.rejected, (state, action) => {
				state.isLoading = false;
				state.isError = true;
				state.isSuccess = false;
				state.message = action.payload;
